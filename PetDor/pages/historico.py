"""
📊 Página de Histórico de Avaliações de Dor do Pet - PETDor

Responsável por:
- Exibir o histórico de avaliações de dor de todos os pets do tutor logado.
- Permitir a visualização detalhada de cada avaliação.
- Permitir a exclusão de avaliações.
"""

import sys
from pathlib import Path
from typing import Dict, Any

# Adiciona a raiz do projeto ao path
root_path = Path(__file__).parent.parent
if str(root_path) not in sys.path:
    sys.sys.path.insert(0, str(root_path))

import streamlit as st
from auth.user import buscar_usuario_por_id
from database.models import buscar_avaliacoes_usuario, deletar_avaliacao, buscar_avaliacao_por_id, buscar_respostas_avaliacao
from config import APP_CONFIG

# Configuração da página
st.set_page_config(
    page_title="Histórico - " + APP_CONFIG["titulo"],
    page_icon="📊",
    layout="centered"
)

def exibir_detalhes_avaliacao(avaliacao_id: int):
    """Exibe os detalhes de uma avaliação específica em um expander."""
    avaliacao = buscar_avaliacao_por_id(avaliacao_id)
    if not avaliacao:
        st.error("Avaliação não encontrada.")
        return

    st.subheader(f"Detalhes da Avaliação de {avaliacao['pet_nome']} em {avaliacao['data_avaliacao'].split('T')[0]}")

    col1, col2 = st.columns(2)
    with col1:
        st.metric("Nível de Dor Registrado", f"{avaliacao['percentual_dor']}%")
        st.write(f"**Pet:** {avaliacao['pet_nome']} ({avaliacao['pet_especie']})")
        st.write(f"**Data da Avaliação:** {avaliacao['data_avaliacao'].split('T')[0]} às {avaliacao['data_avaliacao'].split('T')[1][:5]}")
    with col2:
        st.write(f"**Raça:** {avaliacao['pet_raca'] or 'Não informada'}")
        st.write(f"**Nascimento:** {avaliacao['pet_data_nascimento'] or 'Não informado'}")
        st.write(f"**Sexo:** {avaliacao['pet_sexo'] or 'Não informado'}")
        st.write(f"**Peso:** {avaliacao['pet_peso']} kg" if avaliacao['pet_peso'] else 'Não informado')

    if avaliacao['observacoes']:
        st.markdown(f"**Observações:** {avaliacao['observacoes']}")
    else:
        st.info("Nenhuma observação registrada para esta avaliação.")

    # Exibir respostas detalhadas das perguntas
    respostas = buscar_respostas_avaliacao(avaliacao_id)
    if respostas:
        st.markdown("---")
        st.markdown("#### Respostas Detalhadas:")
        for r in respostas:
            st.write(f"- **{r['pergunta_id'].replace('_', ' ').capitalize()}:** {r['resposta']}")
    else:
        st.info("Nenhuma resposta detalhada de perguntas encontrada para esta avaliação.")

    st.markdown("---")
    if st.button(f"🗑️ Deletar Avaliação de {avaliacao['pet_nome']}", key=f"deletar_avaliacao_{avaliacao_id}", type="secondary"):
        if deletar_avaliacao(avaliacao_id):
            st.success("Avaliação deletada com sucesso!")
            st.rerun()
        else:
            st.error("Erro ao deletar avaliação.")


def main():
    """Renderiza a página de histórico de avaliações."""

    # 1. Verifica se o usuário está logado
    if 'usuario_id' not in st.session_state:
        st.warning("Você precisa estar logado para acessar esta página.")
        st.info("Faça login ou crie uma conta para continuar.")
        st.markdown("""
        <a href="/login" target="_self">
            <button style="background: #4CAF50; color: white; padding: 12px 24px; 
                           border: none; border-radius: 8px; font-size: 16px; 
                           cursor: pointer; width: 100%; margin-top: 1rem;">
                🔐 Ir para Login
            </button>
        </a>
        """, unsafe_allow_html=True)
        st.stop()

    usuario_id = st.session_state['usuario_id']
    usuario_data = buscar_usuario_por_id(usuario_id)
    nome_usuario = usuario_data['nome'] if usuario_data else "Usuário"

    st.markdown(f"""
    <div style="text-align: center; padding: 2rem 1rem;">
        <h1 style="color: #2d3748; margin-bottom: 0.5rem;">📊 Histórico de Avaliações</h1>
        <p style="color: #718096; font-size: 1.1rem;">
            Olá, {nome_usuario}! Veja o histórico de avaliações de dor dos seus pets.
        </p>
    </div>
    """, unsafe_allow_html=True)

    st.markdown("---")

    avaliacoes = buscar_avaliacoes_usuario(usuario_id)

    if not avaliacoes:
        st.info("Você ainda não tem avaliações de dor registradas para seus pets.")
        st.markdown("""
        <a href="/avaliacao" target="_self">
            <button style="background: #2196F3; color: white; padding: 12px 24px; 
                           border: none; border-radius: 8px; font-size: 16px; 
                           cursor: pointer; width: 100%; margin-top: 1rem;">
                📋 Fazer Nova Avaliação
            </button>
        </a>
        """, unsafe_allow_html=True)
        st.stop()

    st.markdown("### Suas Últimas Avaliações")

    for avaliacao in avaliacoes:
        with st.expander(f"**{avaliacao['pet_nome']}** ({avaliacao['pet_especie']}) - {avaliacao['data_avaliacao'].split('T')[0]} - Dor: **{avaliacao['percentual_dor']}%**"):
            exibir_detalhes_avaliacao(avaliacao['avaliacao_id'])

if __name__ == "__main__":
    main()
